...
async def get_chat_notes_for_pdf(pdf_filename: str, page_number: Optional[int] = None) -> List[ChatNoteResponse]:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting chat notes: {str(e)}")
...
async def get_chat_note_by_id(note_id: int) -> ChatNoteResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting chat note: {str(e)}")
...
async def delete_chat_note(note_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error deleting chat note: {str(e)}")